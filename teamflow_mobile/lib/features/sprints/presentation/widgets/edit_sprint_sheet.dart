...
class EditSprintSheet extends HookConsumerWidget {
  final SprintModel sprint;

  const EditSprintSheet({super.key, required this.sprint});

  @override
  Widget build(BuildContext context, WidgetRef ref) {
    final nameCtrl = useTextEditingController(text: sprint.name);
    final goalCtrl = useTextEditingController(text: sprint.goal ?? '');
    final startDate = useState<DateTime>(sprint.startDate);
    final endDate = useState<DateTime>(sprint.endDate);
    ...
    void handleSave() async {
      ...
      // Check duplication excluding this sprint
      final sprints = ref.read(sprintsListProvider(sprint.projectId)).value ?? [];
      final isDuplicate = sprints.any((s) => s.id != sprint.id && s.name.toLowerCase() == name.toLowerCase());
      if (isDuplicate) {
        showAppSnackBar(context, 'Sprint name already exists in this project');
        return;
      }

      final success = await ref.read(sprintControllerProvider.notifier).updateSprint(
            sprint.id,
      ...
      child: Container(
        ...
        child: Column(
          ...
          children: [
            ...
            ElevatedButton(
              onPressed: controllerState.isLoading ? null : handleSave,
